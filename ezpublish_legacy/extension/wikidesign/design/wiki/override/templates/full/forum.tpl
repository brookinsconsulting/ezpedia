...
<div class="content-view-full">
   ...
   {let forum=$node.parent}
    ...
    {* <div class="content-navigator-forum-link"> *}
    <p><a href={$node.url_alias|ezurl}>{$node.name|wash}</a> for article <a href={$forum.parent.url_alias|ezurl}>{$forum.parent.name|wash}</a>: <a href={$forum.url|ezurl}>{$forum.name|wash}</a></p>
    </div>
   {/let}
        ...
        <tr class="{$topic.sequence}">
            ...
            {let last_reply=fetch('content','list',hash( parent_node_id, $topic.node_id,
                ...
                <div class="attribute-byline">
                   ...
                   <a href={concat( $reply.parent.url_alias, '/(offset)/', sub( $topic_reply_count, mod( $topic_reply_count, 20 ) ) , '#msg', $reply.node_id )|ezurl}>{$reply.object.published|l10n(shortdatetime)}</a>
                   {section-else}
                   <a href={concat( $reply.parent.url_alias, '#msg', $reply.node_id )|ezurl}>{$reply.object.published|l10n(shortdatetime)}</a>
                   {/section}
                   </p>
